...
Above are summaries of each document.
There are two approaches to summarising all the documents in the entire corpus. 
1. Run summarisisation again, but on the entire corpus
1. Just summarise the existing summaries

(MAYBE EVEN DO IT FOR KEYWORDS?)

```python
s = " "
s=s.join(Corpus_of_Summaries)
t=summarize(s, ratio=0.03)
print(t)
